...
void computeHash(const string& hashProgName)
{
	...
	//Close write end on PC pipe 
	if(close(parentToChildPipe[WRITE_END]) < 0){
		perror("Error closing write end on PC pipe");
		exit(-1);
	}

	//Close read end of CP pipe
	if(close(childToParentPipe[READ_END]) < 0){
		perror("Error closing read end on CP pipe");
		exit(-1);
	}
	...
	//Close read end of PC pipe  
	if(close(parentToChildPipe[READ_END]) < 0){
		perror("Error closing read end on PC pipe");
		exit(-1);
	}
	...
	const char * cmdline = cmdLine.c_str();
...
void parentFunc(const string& hashProgName)
{
	...
	//Close read end on PC pipe
	if(close(parentToChildPipe[READ_END]) < 0){
		perror("Error closing read end on PC pipe");
		exit(-1);
	}

	//Close write end on CP pipe
	if(close(childToParentPipe[WRITE_END]) < 0){
		perror("Error closing write end on CP pipe");
		exit(-1);
	}
